...
```bash
node tests/engine-walk.js     # 12 control-flow checks, no API calls
```

Covers multi-item loops, `askIf` branches inside loop items, `back()` discarding
a speculatively-opened item, `jumpTo` into a specific loop index, save/resume
round-trips, and a throwing `askIf` not stranding the interview.
